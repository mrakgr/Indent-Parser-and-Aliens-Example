// The only thing #8 lacks is informative error messages
...
let many1Indents (p: Parser<IndentTypes,_>) (stream: CharStream<UserState>) =
    ...
    if stream.UserState.indent < indent then // Go up an indent level
        ...
        if result.Status <> Ok then result 
        else
            while // A do while loop in F#.
                (
                results.Add(result.Result)
                if indent = indent2 then
                    if stream.UserState.indent <> indent then stream.UserState <- UserState(indent,depth) // Sets the userstate.indent to the current level in case it has been modified.
                    result <- p stream
                    indent2 <- getIndent()
                    result.Status = Ok
                else false)
            ...
            if stream.IsEndOfStream then Reply(Statements <| results.ToArray())
            else if result.Status <> Ok then result
            else if indent2 < indent && depth = 0 then Reply(Error, messageError "wrong indentation")
            else if indent2 > indent then Reply(Error, messageError "wrong indentation")
            else Reply(Statements <| results.ToArray())
    else 
        Reply(Error, expected "up indent")
